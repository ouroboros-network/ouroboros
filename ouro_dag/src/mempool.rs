...
static MEV_BATCH_ORDERING: Lazy<Arc<BatchOrdering>> = Lazy::new(|| {
    Arc::new(BatchOrdering::new(5))
});
...
impl Mempool {
 ...
 log::warn!("Mempool full ({} transactions) - enforcing eviction policy", current_size);
 ...
 lowest_fee_tx.id, lowest_fee_tx.fee, txn.id, txn.fee
 );
 }
 Err(e) => {
 log::error!("Failed to evict transaction: {}", e);
 return Err(IoError::new(ErrorKind::Other, "Failed to evict low-fee transaction"));
 }
 }
 } else {
 ...
 put(&self.db, key.into_bytes(), txn).map_err(|e| {
 IoError::new(ErrorKind::Other, format!("db put error: {}", e))
 })?;

 log::debug!(
 "Added transaction {} to mempool (fee: {}, size: {}/{})",
 txn.id, txn.fee, current_size + 1, MAX_MEMPOOL_SIZE
 );
 ...
 tier_b.cmp(&tier_a) // Higher tiers first
 .then_with(|| {
 ...
 log::debug!("Selected {} transactions with MEV protection", selected.len());
