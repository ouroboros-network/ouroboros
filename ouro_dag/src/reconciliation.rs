...
use crate::storage;
use crate::vm;
use crate::tail_emission::{EmissionConfig, calculate_block_reward};
use crate::fee_market::FeeMarketManager;
use crate::account_abstraction::{EntryPoint, UserOperation};
use crate::indexer::{Indexer, IndexedBlock, IndexedTransaction};
...
static FEE_MARKET: Lazy<Arc<Mutex<FeeMarketManager>>> = Lazy::new(|| {
    Arc::new(Mutex::new(FeeMarketManager::new()))
});

/// Global entry point for account abstraction
static ACCOUNT_ABSTRACTION: Lazy<Arc<Mutex<EntryPoint>>> = Lazy::new(|| {
    Arc::new(Mutex::new(EntryPoint::new()))
});

/// Global blockchain indexer
static INDEXER: Lazy<Arc<Indexer>> = Lazy::new(|| {
    Arc::new(Indexer::new())
});
...
pub async fn finalize_block(block_id: Uuid) -> Result<()> {
 ...
 Ok(Err(e)) => {
 warn!("WARNING Failed to load transaction {}: {} - skipping", tx_id, e);
 continue;
 }
 Err(e) => {
 ...
 warn!("WARNING No transactions could be loaded for block {}", block_id);
 return Ok(());
 }
 ...
 info!(" Executing smart contracts for {} transactions...", transactions.len());
 ...
 info!(" Fee market: base_fee = {} (gas used: {})", current_base_fee, total_gas);
...
async fn index_block_data(
    ...
    info!(" Indexed {} transactions for block {}", transactions.len(), block_id);
