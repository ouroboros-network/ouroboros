...
use super::economics::{FeeDistribution, FeeAllocation};
...
impl FeeProcessor {
    ...
    pub fn with_distribution(treasury_address: String, distribution: FeeDistribution) -> Result<Self> {
 ...
 if let Some(treasury_transfer) = transfers.iter_mut().find(|t| t.purpose == TransferPurpose::Treasury) {
 ...
 Ok(FeeProcessingResult {
    ...
    pub fn execute_transfers(&self, result: &FeeProcessingResult, fee_payer: &str) -> Result<()> {
        ...
        if payer_balance < result.total_fee {
            bail!("Insufficient balance: {} has {} but fee is {}", fee_payer, payer_balance, result.total_fee);
        }
        ...
        if result.burned_amount > 0 {
            ...
            let current_burned: u64 = crate::storage::get_str::<u64>(db, burned_key)
                .unwrap_or(Some(0))
                .unwrap_or(0);
            crate::storage::put_str(db, burned_key, &(current_burned + result.burned_amount).to_string())
                .map_err(|e| anyhow::anyhow!("Failed to update burned total: {}", e))?;

            log::info!("Burned {} units from fee (total burned: {})", result.burned_amount, current_burned + result.burned_amount);
        }
    ...
    pub fn execute_aggregated(&self, aggregated: &AggregatedFees, fee_payers: &[(String, u64)]) -> Result<()> {
        let db = self.db.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Database handle not set - cannot execute transfers"))?;
        ...
        for (payer, amount) in fee_payers {
            ...
            if payer_balance < *amount {
                bail!("Insufficient balance: {} has {} but owes {}", payer, payer_balance, amount);
            }
        ...
        self.set_balance(db, &self.treasury_address, treasury_balance + aggregated.treasury_total)?;
        ...
        if aggregated.burned_total > 0 {
            ...
            let current_burned: u64 = crate::storage::get_str::<u64>(db, burned_key)
                .unwrap_or(Some(0))
                .unwrap_or(0);
            crate::storage::put_str(db, burned_key, &(current_burned + aggregated.burned_total).to_string())
                .map_err(|e| anyhow::anyhow!("Failed to update burned total: {}", e))?;
        }
 ...
 *validator_totals.entry(transfer.recipient.clone()).or_insert(0) += transfer.amount;
 }
 TransferPurpose::Treasury => {
 treasury_total += transfer.amount;
 }
 TransferPurpose::AppDeveloper => {
 *developer_totals.entry(transfer.recipient.clone()).or_insert(0) += transfer.amount;
 }
 }
 }
 }
