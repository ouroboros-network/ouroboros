...
impl MerkleTree {
 ...
 let level: Vec<[u8; 32]> = hashes.iter().map(|h| {
 let mut d = [0u8; 32];
 d.copy_from_slice(&hex::decode(h).expect("bad hash")[..32]);
 d
 }).collect();
 let mut nodes = vec![level.clone()];
 while nodes.last().expect("nodes vector should never be empty").len() > 1 {
 ...
 MerkleTree { nodes, original_leaves: vec![] }
 }

 /// Build Merkle tree from raw byte leaves
 pub fn from_leaves(leaves: &[Vec<u8>]) -> Self {
 if leaves.is_empty() {
 return MerkleTree { nodes: vec![vec![[0u8; 32]]], original_leaves: vec![] };
 }

 let level: Vec<[u8; 32]> = leaves.iter().map(|leaf| {
 let mut hasher = Sha256::new();
 hasher.update(leaf);
 let res = hasher.finalize();
 let mut arr = [0u8; 32];
 ...
 MerkleTree { nodes, original_leaves: leaves.to_vec() }
 }
 ...
 if index >= leaves.len() {
 bail!("Index {} out of bounds (tree has {} leaves)", index, leaves.len());
 }
 ...
 .iter()
 .map(|(s, is_left)| -> Result<(Vec<u8>, bool)> {
 Ok((hex::decode(s)?, *is_left))
 })
...
mod tests {
 ...
 assert_eq!(proof.siblings.len(), 0, "Single leaf should have no siblings");
 }
 ...
 assert!(proof.verify().unwrap(), "Proof verification failed for index {}", i);
 }
 }
 ...
 let siblings_hex: Vec<(String, bool)> = proof.siblings.iter()
