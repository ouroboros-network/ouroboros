...
impl SecurityMode {
 ...
 bail!("Signature from unauthorized key: expected {}, got {}", owner_pubkey, sig.signer_pubkey);
 }
 ...
 // Verify signature
 if self.verify_ed25519_signature(tx_hash, &sig.signature, &sig.signer_pubkey).is_ok() {
 valid_count += 1;
 } else {
 ...
 if sig_bytes.len() != 64 {
 bail!("Invalid signature length: expected 64, got {}", sig_bytes.len());
 }
 ...
 SecurityMode::Federated { authorized_keys, .. } => authorized_keys.clone(),
 }
 }
