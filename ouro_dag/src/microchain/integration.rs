...
pub async fn validate_microchain_transaction(
 ...
 let mode = security_modes
 .get(microchain_id)
 .ok_or_else(|| anyhow::anyhow!("Microchain {} not found or not registered", microchain_id))?;
...
impl MicrochainRegistration {
 ...
 SecurityMode::Federated { authorized_keys, threshold } => {
...
mod tests {
 ...
 // Validate
 let result = validate_microchain_transaction(store.clone(), "mc1", tx_hash, &signatures).await;
 assert!(result.is_ok());
 }
 ...
 // Validate
 let result = validate_microchain_transaction(store.clone(), "mc2", tx_hash, &signatures).await;
 assert!(result.is_ok());
 }
 ...
 assert!(is_key_authorized(store.clone(), "mc3", &authorized_key).await.unwrap());
 assert!(!is_key_authorized(store.clone(), "mc3", &unauthorized_key).await.unwrap());
 }
