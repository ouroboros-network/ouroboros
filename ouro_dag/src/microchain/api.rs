...
use axum::{Router, routing::{post, get}, Json, extract::State, http::StatusCode, response::IntoResponse, middleware};
...
pub async fn append_header(State(_state): State<MicroState>, Json(_hdr): Json<AppendMicroHeader>) -> impl IntoResponse {
 (StatusCode::OK, Json(ApiOk { status: "ok" }))
}

pub async fn provisional_claim(State(_state): State<MicroState>, Json(_payload): Json<ProvisionalClaim>) -> impl IntoResponse {
...
pub async fn tip(State(_state): State<MicroState>) -> impl IntoResponse {
 (StatusCode::OK, Json(ApiOk { status: "tip-not-implemented" }))
}
