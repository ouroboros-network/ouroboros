...
impl ChallengeManager {
 ...
 let mut anchors = self.pending_anchors.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 ...
 println!(" Challenge period: {} days", MICROCHAIN_CHALLENGE_PERIOD_SECS / 86400);
 ...
 let anchors = self.pending_anchors.read().unwrap_or_else(|poisoned| poisoned.into_inner());
 let anchor = anchors.get(&anchor_hash)
 ...
 let bonds = self.challenge_bonds.read().unwrap_or_else(|poisoned| poisoned.into_inner());
 ...
 let mut challenges = self.challenges.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 ...
 println!("WARNING Challenge submitted against anchor: {:?}", hex::encode(anchor_hash));
 ...
 ChallengeType::DoubleSpend => {
 self.verify_double_spend(&challenge.evidence)
 }
 ChallengeType::StateRootMismatch => {
 self.verify_state_root(&challenge.evidence, microchain_state)
 }
 ChallengeType::InvalidSignature => {
 self.verify_signatures(&challenge.evidence)
 }
 ...
 let mut anchors = self.pending_anchors.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 let anchor = anchors.get_mut(&challenge.anchor_hash)
 .ok_or("Anchor not found")?;
 ...
 let mut anchors = self.pending_anchors.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 let anchor = anchors.get_mut(&anchor_hash)
 .ok_or("Anchor not found")?;
 ...
 let mut exits = self.exit_requests.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 ...
 pub fn process_force_exit(
 &self,
 exit_id: &str,
 current_time: u64,
 ) -> Result<u64, String> {
 let mut exits = self.exit_requests.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 let exit = exits.get_mut(exit_id)
 .ok_or("Exit request not found")?;
 ...
 let mut bonds = self.challenge_bonds.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 let current = bonds.get(&user).copied().unwrap_or(0);
 bonds.insert(user.clone(), current + amount);

 println!("REWARD Challenge bond deposited by {}: {} OURO", user, amount / 100_000_000);
 }

 /// Deposit operator stake
 pub fn deposit_operator_stake(&self, operator: String, amount: u64) {
 let mut stakes = self.operator_stakes.write().unwrap_or_else(|poisoned| poisoned.into_inner());
 let current = stakes.get(&operator).copied().unwrap_or(0);
 stakes.insert(operator.clone(), current + amount);

 println!("REWARD Operator stake deposited by {}: {} OURO", operator, amount / 100_000_000);
 }
 ...
 fn verify_state_root(&self, evidence: &ChallengeEvidence, _state: &HashMap<String, u64>) -> bool {
 ...
 bonds.insert(challenger.to_string(), challenger_bond - CHALLENGE_BOND_AMOUNT);

 println!("FAST Challenger bond slashed: {} OURO", CHALLENGE_BOND_AMOUNT / 100_000_000);
 ...
 let challenges = self.challenges.read().unwrap_or_else(|poisoned| poisoned.into_inner());
...
mod tests {
 ...
 assert!(result.is_ok());
 assert_eq!(manager.get_anchor_status(anchor_hash), Some(AnchorStatus::Challenged));
 }
