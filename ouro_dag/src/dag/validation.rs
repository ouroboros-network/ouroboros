...
pub fn validate_transaction(
 ...
 let total_debit = txn.amount.checked_add(txn.fee)
 ...
 return Err(format!("Transaction {} already exists (double-spend attempt)", txn.id));
 }
...
mod tests {
 ...
 assert!(result.unwrap_err().contains("Sender address cannot be empty"));
 }
