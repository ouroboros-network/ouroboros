...
pub mod pq; // Post-quantum primitives (Dilithium5, Kyber1024)
pub mod hybrid; // Hybrid signatures (Ed25519 + Dilithium5)
...
pub fn load_signing_key(path: &Path) -> Result<SigningKey> {
 ...
 let raw_array: [u8; 64] = raw.as_slice().try_into().map_err(|_| anyhow::anyhow!("invalid key bytes"))?;
 let sk = SigningKey::from_keypair_bytes(&raw_array).map_err(|e| anyhow::anyhow!("invalid keypair bytes: {}", e))?;
 Ok(sk)
}
