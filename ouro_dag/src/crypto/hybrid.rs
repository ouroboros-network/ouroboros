...
impl HybridSignature {
 ...
 pub fn verify(&self, message: &[u8], ed25519_pubkey: &VerifyingKey, dilithium_pubkey: &DilithiumPublicKey) -> Result<()> {
 // Verify Ed25519 signature
 let ed_sig = Ed25519Signature::from_bytes(
 self.ed25519.as_slice().try_into()
 .map_err(|_| anyhow::anyhow!("Invalid Ed25519 signature length"))?
 );

 ed25519_pubkey.verify(message, &ed_sig)
...
impl HybridKeypair {
 ...
 signature.verify(message, &self.ed25519_keypair.1, &self.dilithium_keypair.public_key)
 }
...
pub fn verify_with_migration_policy(
 ...
 return ed_pk.verify(message, &sig)
 .map_err(|e| anyhow::anyhow!("Ed25519 verification failed: {}", e));
 }
 }
 ...
 MigrationPhase::Phase2HybridOnly => {
 // Hybrid required
 let ed_pk = ed25519_pubkey.ok_or_else(|| anyhow::anyhow!("Ed25519 pubkey required for hybrid"))?;
 let dil_pk = dilithium_pubkey.ok_or_else(|| anyhow::anyhow!("Dilithium pubkey required for hybrid"))?;
 ...
 MigrationPhase::Phase3DilithiumOnly => {
 // Pure Dilithium only
 let dil_pk = dilithium_pubkey.ok_or_else(|| anyhow::anyhow!("Dilithium pubkey required"))?;
...
mod tests {
 ...
 assert_eq!(MigrationPhase::current_phase(0), MigrationPhase::Phase1EdOrHybrid);
 assert_eq!(MigrationPhase::current_phase(500_000), MigrationPhase::Phase1EdOrHybrid);
 assert_eq!(MigrationPhase::current_phase(1_000_000), MigrationPhase::Phase2HybridOnly);
 assert_eq!(MigrationPhase::current_phase(3_000_000), MigrationPhase::Phase2HybridOnly);
 assert_eq!(MigrationPhase::current_phase(5_000_000), MigrationPhase::Phase3DilithiumOnly);
 ...
 println!(" Overhead ratio: {:.1}x", hybrid_sig.size_bytes() as f64 / ed_sig.to_bytes().len() as f64);
 }
}
