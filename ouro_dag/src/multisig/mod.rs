...
impl MultiSigConfig {
 ...
 pub fn new(
 threshold: usize,
 validator_pubkeys: HashMap<String, VerifyingKey>,
 ) -> Result<Self> {
...
impl MultiSignature {
 ...
 if self.partial_signatures.iter().any(|p| p.validator_id == partial.validator_id) {
 bail!("Duplicate signature from validator {}", partial.validator_id);
 }
 ...
 let pubkey = config.validator_pubkeys.get(&partial.validator_id)
 ...
 partial.signature.as_slice().try_into()
...
impl MultiSigCoordinator {
 ...
 let multisig = pending.entry(anchor_root.clone())
