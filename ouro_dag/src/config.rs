...
fn validate_rocksdb_path(validation: &mut ConfigValidation) {
 ...
 format!("Cannot create RocksDB directory '{}': {}", rocksdb_path, e)
 );
 } else {
 ...
 *key == "test" {
 ...
 _ => {
 validation.add_warning(
 "API_KEYS not set - Node will run without API authentication".into()
 );
 }
 }
}
   ...
   free_data_feeds: DataFeedsConfig {
    ...
    weather: WeatherFeedConfig {
     ...
     default_locations: vec![
      LocationConfig { name: "New York".into(), lat: 40.7128, lon: -74.0060 },
     ],
     refresh_ms: 300000,
    },
