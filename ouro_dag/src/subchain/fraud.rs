...
impl FraudProofManager {
 ...
 Ok(fraud_proof.id)
    ...
    async fn verify_invalid_merkle_root(&self, _proof: &FraudProof) -> Result<FraudVerificationResult> {
    ...
    pub async fn verify_fraud_proof(&self, fraud_proof_id: Uuid) -> Result<FraudVerificationResult> {
        // Fetch fraud proof
        let fraud_proof = self.get_fraud_proof(fraud_proof_id).await?
            .ok_or_else(|| anyhow::anyhow!("Fraud proof not found"))?;

        // Verify based on fraud type
        let result = match fraud_proof.fraud_type {
            FraudType::InvalidMerkleRoot => {
                self.verify_invalid_merkle_root(&fraud_proof).await?
            }
            FraudType::MissingTransaction => {
                self.verify_missing_transaction(&fraud_proof).await?
            }
            FraudType::InvalidTransaction => {
                self.verify_invalid_transaction(&fraud_proof).await?
            }
            FraudType::DoubleInclusion => {
                self.verify_double_inclusion(&fraud_proof).await?
            }
            FraudType::InvalidAttestation => {
                self.verify_invalid_attestation(&fraud_proof).await?
            }
        };
 ...
 /// }
 async fn verify_missing_transaction(&self, proof: &FraudProof) -> Result<FraudVerificationResult> {
 ...
 let tx_found = actual_txs.iter().any(|tx| {
 tx.as_str().map(|s| s == claimed_tx_id).unwrap_or(false)
 });
 ...
 format!("Transaction {} was claimed but is missing from batch", claimed_tx_id)
 } else if tx_found {
 ...
 async fn verify_invalid_transaction(&self, proof: &FraudProof) -> Result<FraudVerificationResult> {
 ...
 async fn verify_invalid_attestation(&self, proof: &FraudProof) -> Result<FraudVerificationResult> {
 ...
 "Attestation is valid".to_string()
    ...
    async fn execute_slashing(&self, _proof: &FraudProof, _result: &FraudVerificationResult) -> Result<()> {
