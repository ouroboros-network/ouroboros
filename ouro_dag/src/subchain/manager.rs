...
use crate::subchain::store::{SubStore, SubBlockHeader, BatchRecord};
use uuid::Uuid;
use anyhow::Result;
use chrono::Utc;
use std::sync::Arc;
...
impl SubchainManager {
 ...
 let u = Url::parse(sref).map_err(|e| anyhow::anyhow!("invalid serialized_leaves_ref url: {}", e))?;
 if u.scheme() != "file" {
 return Err(anyhow::anyhow!("unsupported serialized_leaves_ref scheme"));
 }
 let path = u.to_file_path().map_err(|_| anyhow::anyhow!("invalid file URL"))?;
 let data = fs::read_to_string(&path)
 .map_err(|e| anyhow::anyhow!("failed reading serialized leaves file {}: {}", path.display(), e))?;
 let leaves: Vec<MicroAnchorLeaf> = serde_json::from_str(&data)
 .map_err(|e| anyhow::anyhow!("failed parsing serialized leaves json {}: {}", path.display(), e))?;
