...
use anyhow::Result;
use uuid::Uuid;
use crate::multisig::{MultiSigCoordinator, PartialSignature, MultiSignature};
...
impl AggregatorAttestation {
 ...
 // Parse public key
 let pubkey_array: [u8; 32] = self.aggregator_pubkey
 .as_slice()
 .try_into()
 ...
 // Parse signature
 let sig_array: [u8; 64] = self.signature
 .as_slice()
 .try_into()
...
impl AnchorService {
 ...
 let coordinator = self.multisig.as_ref()
 ...
 let is_complete = coordinator.submit_partial_signature(
 root.to_vec(),
 subchain,
 block_height,
 partial,
 ...
 let multisig = coordinator.get_completed_multisig(root)
 ...
 let txid = self.post_multisig_anchor(subchain, block_height, root, &multisig).await?;
 ...
 pub async fn post_anchor(&self, subchain: Uuid, block_height: i64, root: &[u8]) -> Result<String> {
 ...
 let tree = Self::create_merkle_tree_from_data(&tx_hashes.iter().map(|s| s.as_bytes().to_vec()).collect::<Vec<_>>());
