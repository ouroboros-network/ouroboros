...
use super::gas::{GasState, hash_gas_cost, copy_gas_cost};
use super::precompiles::Precompiles;
use super::storage::ContractStorage;
use super::types::{ContractAddress, StorageKey, ContractLog};
use anyhow::Result;
...
fn register_storage_functions(
 ...
 "env",
 "storage_set",
 move |mut caller: Caller<GasState>, key_ptr: u32, key_len: u32, value_ptr: u32, value_len: u32| -> Result<(), wasmi::core::Trap> {
 // Charge gas
 let gas_cost = caller.data().costs.storage_set;
 caller.data_mut().consume_gas(gas_cost)
 ...
 // Store
 ctx.storage.set_storage(storage_key, value_bytes)
 .map_err(|e| wasmi::core::Trap::new(e.to_string()))?;
 ...
 "env",
 "storage_get",
 move |mut caller: Caller<GasState>, key_ptr: u32, key_len: u32, value_ptr: u32| -> Result<u32, wasmi::core::Trap> {
 // Charge gas
 ...
 // Get value
 match ctx.storage.get_storage(&storage_key)
 .map_err(|e| wasmi::core::Trap::new(e.to_string()))?
 {
 Some(value) => {
...
fn register_crypto_functions(linker: &mut Linker<GasState>) -> Result<()> {
 ...
 "env",
 "sha256",
 |mut caller: Caller<GasState>, data_ptr: u32, data_len: u32, output_ptr: u32| -> Result<(), wasmi::core::Trap> {
 // Charge gas
 ...
 "env",
 "ed25519_verify",
 |mut caller: Caller<GasState>, pubkey_ptr: u32, sig_ptr: u32, msg_ptr: u32, msg_len: u32| -> Result<u32, wasmi::core::Trap> {
...
fn register_env_functions(
 ...
 "env",
 "get_block_number",
 move |_caller: Caller<GasState>| -> u64 {
 ctx.block_number
 },
 )?;
...
fn register_log_functions(
 ...
 "env",
 "log",
 move |mut caller: Caller<GasState>, data_ptr: u32, data_len: u32| -> Result<(), wasmi::core::Trap> {
 // Charge gas for logging (8 gas per byte)
 caller.data_mut().consume_gas(data_len as u64 * 8)
 .map_err(|e| wasmi::core::Trap::new(e.to_string()))?;
...
fn register_utility_functions(linker: &mut Linker<GasState>) -> Result<()> {
 ...
 Err(wasmi::core::Trap::new(format!("Contract reverted: {}", msg)))
 },
 )?;
