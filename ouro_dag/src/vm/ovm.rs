...
impl OuroborosVM {
 ...
 0 => {
 // No args function
 let typed_func = func.typed::<(), u32>(&store)
 .map_err(|e| anyhow::anyhow!("Invalid function signature: {}", e))?;
 typed_func.call(&mut store, ())
 }
 2 => {
 // (ptr, len) function
 let typed_func = func.typed::<(u32, u32), u32>(&store)
 .map_err(|e| anyhow::anyhow!("Invalid function signature: {}", e))?;
 ...
 memory.read(&store, (result_ptr + 4) as usize, &mut data)
 ...
 pub fn get_contract_metadata(&self, address: ContractAddress) -> Result<Option<ContractMetadata>> {
 ...
 log::info!("DEBUG: Validating WASM module for determinism (size: {} bytes)", wasm_bytes.len());
 ...
 Operator::F32ReinterpretI32 => float_ops_found.push("f32.reinterpret_i32"),
 Operator::F64ReinterpretI64 => float_ops_found.push("f64.reinterpret_i64"),
 Operator::I32ReinterpretF32 => float_ops_found.push("i32.reinterpret_f32"),
 Operator::I64ReinterpretF64 => float_ops_found.push("i64.reinterpret_f64"),
 ...
 let import = import.map_err(|e| anyhow::anyhow!("Failed to read import: {}", e))?;
 ...
 log::error!(" Found operations: {:?}", &float_ops_found[..std::cmp::min(10, float_ops_found.len())]);
 anyhow::bail!(
 ...
 log::warn!(" Ensure all imported functions are deterministic and registered as host functions");
 }
 ...
 if pages_to_grow > 0 {
 memory.grow(&mut *store, wasmi::core::Pages::new(pages_to_grow as u32).unwrap())
...
mod tests {
 ...
 let address = vm.deploy_contract(
 &wasm,
