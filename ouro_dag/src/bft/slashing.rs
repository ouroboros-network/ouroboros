...
impl SlashingManager {
 ...
 validator_id, reason, severity
 );
 log::error!(
 " Stake before: {} units, slashing: {} units ({}%), remaining: {} units",
 stake_before, slashed_amount, penalty_pct, stake_after
 );
 log::error!(" Evidence: {}", evidence);
        ...
        log::error!(
            ...
            validator_id, reason, slashed_amount
        );
 ...
 /// Get validator stake
    ...
    pub async fn get_recent_slashing_events(&self, limit: i64) -> Result<Vec<SlashingEvent>> {
        ...
        for event_key in index.iter().skip(start).rev() {
            if let Ok(Some(event)) = crate::storage::get::<_, SlashingEvent>(&self.db, event_key.as_bytes()) {
                events.push(event);
    ...
    pub async fn get_slashing_history(&self, validator_id: &str) -> Result<Vec<SlashingEvent>> {
        ...
        match crate::storage::get::<_, Vec<SlashingEvent>>(&self.db, key.as_bytes()).map_err(|e| anyhow::anyhow!(e))? {
