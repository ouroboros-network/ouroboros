...
impl WalletLink {
    ...
    pub fn create(
        ...
        let message = format!(
            ...
            node_public_key, wallet_address, linked_at
        );
    ...
    pub fn verify_node_signature(&self) -> Result<bool, Box<dyn Error>> {
        ...
        let message = format!(
            "Link node {} to wallet {} at {}",
            self.node_public_key, self.wallet_address, self.linked_at
        );
...
mod tests {
    ...
    fn test_create_wallet_link() {
        ...
        let link = WalletLink::create(wallet_addr.clone(), &node_keypair, wallet_sig).unwrap();
    ...
    fn test_save_and_load_wallet_link() {
        ...
        let link =
            WalletLink::create("ouro1test".to_string(), &node_keypair, "sig".to_string()).unwrap();
